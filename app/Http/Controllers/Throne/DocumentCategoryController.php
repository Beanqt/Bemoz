...
class DocumentCategoryController extends MediaManagerController
{
    public $html;
    public $deleteCache = [
        'document_category.{id}',
        'document_items.{id}',
        'document_category.folder.{boss}',
    ];
    ...
    public function index() {
        ...
        $files = DocumentItem::where('category', $this->boss[1])->orderBy('order', 'asc')->orderBy('id', 'asc')->get();
        $this->getBreadCrumbs($this->boss[1], 'documents');
    ...
    public function create(Request $request) {
        ...
        if($request->isMethod('post')) {
            ...
            if($this->model->isValid($e)){
                ...
                $return['type'] = 'folder';
                $return['item'] = view('throne.widgets.media_manager.documents._folder_item')->with('folder', $model)->render();
            }else{
    ...
    public function edit(Request $request) {
        ...
        $documentcategory = $this->model->find($id);
        $return['title'] = trans('admin.'.$this->default.'.edit');
        $return['content'] = trans('admin.'.$this->default.'.alerts.not');

        if($documentcategory) {
            View::share('edit', $id);
            View::share('data', $documentcategory);

            if($request->isMethod('post')) {
                $e = request()->all();
                $e['boss'] = $this->boss[1];

                if($this->model->isValid($e, $documentcategory)){
                    $model = $this->model->saveData($e, $documentcategory);
                    ...
                    $return['message'] = ['type' => 'error', 'msg' => trans('admin.alert.haveError')];
                    $e['image'] = $documentcategory['image'];
                    $e['image_crop'] = $documentcategory['image_crop'];
                    View::share('data', $e);
                }
            }
