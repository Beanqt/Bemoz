...
class DocumentItemController extends MediaManagerController
{
    public $deleteCache = [
        'documents.{id}',
    ...
    public function __construct(DocumentItem $model){
        $this->model = $model;
    ...
    public function edit(Request $request) {
        ...
        $folder = DocumentCategory::find($this->boss[1]);
        $return['title'] = trans('admin.'.$this->default.'.edit');
        $return['content'] = trans('admin.documentcategory.alerts.not');
            ...
            if($document){
                ...
                if ($request->isMethod('post')) {
                    ...
                    if($this->model->isValid($e, $document)){
                        ...
                        $return['type'] = 'file';
                        $return['item'] = view('throne.widgets.media_manager.documents._item')->with('item', $model)->render();
                    }else{
                        $return['message'] = ['type' => 'error', 'msg' => trans('admin.alert.haveError')];
                        $e['file'] = $document['file'];
                ...
                if(!isset($return['close'])){
                    $return['var'] = $this->model->validator;
