...
class DocumentCategory extends Model {
    ...
    public static $name = 'documentcategory';
    ...
    public function saveData($e, $data = null){
        ...
        if(isset($e['removeImage']) && $e['removeImage']==1 && $data){
            if(!empty($data['image'])){
                $image->remove('documentitem', $data['image']);
            }
            $model->image = '';
            $model->image_crop = '';
        }
        if($image->hasImage()){
            $image->save('documentitem');
    ...
    public function documentitem(){
        return $this->hasMany(DocumentItem::class,'category','id')->where('active',1)->orderBy('order','asc');
    }
}
