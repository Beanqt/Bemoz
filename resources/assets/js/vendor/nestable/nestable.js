...
(function($, window, document, undefined) {
    ...
    Plugin.prototype = {
        ...
        dragStart: function(e) {
            ...
            var ckEditor = dragItem.find('.ck_textarea');
            if(ckEditor.length > 0) {
                $.each(ckEditor, function(element){
                    var ckId = $(this).attr('id');
                    var editorInstance = CKEDITOR.instances[ckId];
                    ckEditor.val(editorInstance.getData());
                    editorInstance.destroy(true);
                    CKEDITOR.remove( ckId );
                });
            }
        ...
        dragStop: function(e) {
            ...
            var ckEditor = el.find('.ck_textarea');
            if(ckEditor.length > 0) {
                $.each(ckEditor, function(element){
                    var ckId = $(this).attr('id');
                    var editorInstance = CKEDITOR.instances[ckId];
