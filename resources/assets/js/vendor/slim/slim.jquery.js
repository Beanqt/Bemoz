...
// gets canvas, options passed as arguments:
        ...
        loadImage.setSlimOptions = function(options){
            this.options = options;
        }

        loadImage.getSlimOptions = function(){
            return this.options;
        }
        loadImage.getExtension = function(){
...
// Canvas render method, allows to implement a different rendering algorithm:
        loadImage.renderImageToCanvas = function (
            ...
            var options = loadImage.getSlimOptions();
            var minSize = options.minSize;

            //if(minSize.width > minSize.height && img.width <= img.height){
            //    canvas.width = img.height/(minSize.height/minSize.width);
            //    destX = (canvas.width-img.width)/2;
            //    this.extension = 'png';
            //}else if(minSize.width < minSize.height && img.width >= img.height){
            //    canvas.height = img.width/(minSize.width/minSize.height);
            //    destY = (canvas.height-img.height)/2;
            //    this.extension = 'png';
            //}else if(minSize.width < minSize.height && minSize.width/minSize.height < img.width/img.height){
            //    canvas.height = img.height/(minSize.width/minSize.height);
            //    destY = (canvas.height-img.height)/2;
            //    this.extension = 'png';
            //}
...
// events
        ...
        var getImageAsCanvas = function getImageAsCanvas(src, size, callback, options) {
            ...
            loadImage.setSlimOptions(options);
        ...
        var flattenData = function flattenData(obj) {
            ...
            if(loadImage.getExtension()){
                forcedType = loadImage.getExtension();
            }
            ...
            if (inArray('input', props) && !async) {
                if(forcedType != 'png'){
                    ...
                    data.input.name = forcedType ? getFileNameWithoutExtension(obj.input.name) + '.' + forcedType : obj.input.name,
                    data.input.image = toDataURL(obj.input.image, MimeTypes[forcedType] || obj.input.type);
                    data.input.type = getMimeTypeFromDataURI(data.input.image);

                    if (data.input.type === 'image/png') {
            ...
            _createClass(Slim, [{
                ...
                value: function _manualTransform(callback) {
                    ...
                    this._applyTransforms(this._data.input.image, function (transformedImage) {
                        ...
                        // scale ratio
                        var ratio = _this23._options.devicePixelRatio === 'auto' ? window.devicePixelRatio : _this23._options.devicePixelRatio;
