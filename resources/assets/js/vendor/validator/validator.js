+function ($) {
  ...
  Validator.DEFAULTS = {
    ...
    errors: {
      ...
      filesize: 'Too large file',
      maxfile: 'Too many files',
    },
    feedback: {
  ...
  Validator.VALIDATORS = {
    ...
    'filesize': function ($el) {
      var fileSizeMax = $el.data('filesize')*1024*1024;

      for(var i = 0; i < $el[0].files.length; i++) {
        if($el[0].files[i] && $el[0].files[i].size>fileSizeMax) {
          return true;
        }
      }
    ...
    'maxfile': function ($el) {
      var maxFile = $el.data('maxfile');

      return $el[0].files.length > parseInt(maxFile);
    }
  }
