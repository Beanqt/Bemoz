<div class="form-group {{$errors->first($item['name']) ? 'has-error' : ''}}">
    ...
    @elseif(!empty($item['help']))
        <i class="fas fa-info-circle" data-toggle="tooltip" data-placement="top" title="{{$item['help']}}"></i>
    @endif
    <input class="form-control pickerDateTime" {!! empty($item['date']) ? '' : 'data-date-format="'.$item['date'].'"' !!} name="{{$item['name']}}" id="{{$item['name']}}" type="text" value="{{old($item['name'])}}" {!! empty($item['placeholder']) ? '' : 'placeholder="'.$item['placeholder'].'"' !!} {{$item['required'] ? 'required' : ''}}>
