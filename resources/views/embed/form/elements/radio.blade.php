<div class="form-group {{$errors->first($item['name']) ? 'has-error' : ''}}">
    ...
    @foreach($item['options'] as $key => $option)
        <label class="checkBoxInput radio">
            <input type="radio" name="{{$item['name']}}" id="{{$key}}-{{$item['name']}}" value="{{empty($option['value']) ? $option['title'] : $option['value']}}" {{old($item['name']) == (empty($option['value']) ? $option['title'] : $option['value']) ? 'checked' : ''}} {{$item['required'] ? 'required' : ''}}>
