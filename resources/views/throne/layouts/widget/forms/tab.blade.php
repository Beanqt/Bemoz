        ...
        <div class="panel panel-default active">
            <div class="box">
                <div class="form-group">
                    <label for="title">@lang('admin.widget.tab.form.title')<span class="required">*</span></label>
        ...
        <ul class="dynamic-elements">
            @if(isset($data['data']) && is_array($data['data']))
                @foreach($data['data'] as $item)
                    @include('throne.layouts.widget.elements.tab.add', ['item' => $item])
                @endforeach
            @endif
        </ul>
        <ul class="template hidden">
            @include('throne.layouts.widget.elements.tab.add', ['item' => []])
        </ul>
    </div>
